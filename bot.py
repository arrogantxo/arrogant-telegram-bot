...
CUSTOM_ROLES_PATH = Path(os.getenv("BOT_CUSTOM_ROLES_PATH", str(BASE_DIR / "custom_roles.json")))
...
def load_custom_roles() -> dict[str, Any]:
    if not CUSTOM_ROLES_PATH.exists():
        return {"team": {}, "aliases": {}}
    with CUSTOM_ROLES_PATH.open("r", encoding="utf-8") as f:
        payload = json.load(f)
...
def save_custom_roles(data: dict[str, Any]) -> None:
    CUSTOM_ROLES_PATH.parent.mkdir(parents=True, exist_ok=True)
...
CATEGORY_ALIASES = {
    ...
    "reels": ["reels", "рилс", "рильс", "реелс", "риелс"],
}

ROLE_ALIASES = {
    "manager": ["manager", "менеджер", "руководитель"],
    "montaj": ["montaj", "монтажер", "монтажёр", "мобилограф", "монтаж"],
    "publisher": ["publisher", "публикатор", "публикация", "выкладка", "постинг"],
    ...
    "smm": ["smm", "смм", "контентщик"],
    "assistant1": ["assistant1", "ассистент1", "абдулло"],
    "assistant2": ["assistant2", "ассистент2", "мухаммадаюб"],
}


def role_aliases() -> dict[str, list[str]]:
    aliases = {key: value[:] for key, value in ROLE_ALIASES.items()}
    custom = load_custom_roles().get("aliases", {})
    for key, values in custom.items():
        aliases[key] = list(dict.fromkeys([*aliases.get(key, []), *values]))
    return aliases
...
def team_map() -> dict[str, dict[str, Any]]:
    merged = dict(CONFIG.get("team", {}))
    merged.update(load_custom_roles().get("team", {}))
    return merged
...
def help_text() -> str:
    return (
        ...
        "Классические команды тоже работают:\n"
        "/myid\n"
        "/team\n"
        "/roles\n"
        "/categories\n"
...
def roles_text() -> str:
    lines = ["Текущие роли:"]
    custom_team = load_custom_roles().get("team", {})
    for key, data in team_map().items():
        suffix = " (добавлена через бота)" if key in custom_team else ""
        lines.append(f"- {key}: {data['name']} ({data['chat_id']}){suffix}")
    return "\n".join(lines)


def add_role_from_command(text: str) -> str:
    parts = text.split(" ", 4)
    if len(parts) < 5:
        return (
            "Формат:\n"
    ...
    _, role_key, chat_id_raw, display_block, aliases_block = parts
    role_key = role_key.strip().lower()
    if not re.fullmatch(r"[a-z0-9_]+", role_key):
        return "role_key должен содержать только английские буквы, цифры и _"
    ...
    raw_tail = f"{display_block} {aliases_block}".strip()
    display_name, aliases_raw = (raw_tail.split("|", 1) + [""])[:2]
    display_name = display_name.strip()
    aliases = [item.strip().lower() for item in aliases_raw.split(",") if item.strip()]

    if not display_name:
        return "Нужно указать имя человека."

    data = load_custom_roles()
    data["team"][role_key] = {"name": display_name, "chat_id": chat_id}
    merged_aliases = [role_key, display_name.lower(), *aliases]
    data["aliases"][role_key] = list(dict.fromkeys(merged_aliases))
    ...
    return (
        ...
        f"Имя: {display_name}\n"
        f"chat_id: {chat_id}\n"
        f"Алиасы: {', '.join(data['aliases'][role_key])}"
    )


def remove_role_from_command(text: str) -> str:
    parts = text.split(maxsplit=1)
    if len(parts) < 2:
        return "Формат:\n/delrole role_key\n\nПример:\n/delrole assistant3"

    role_key = parts[1].strip().lower()
    if role_key in CONFIG.get("team", {}):
        return "Эта роль встроена в конфиг. Удалять через бота можно только роли, добавленные через /addrole."

    data = load_custom_roles()
    removed_team = data.get("team", {}).pop(role_key, None)
    data.get("aliases", {}).pop(role_key, None)
    save_custom_roles(data)

    if not removed_team:
        return "Роль не найдена."
    return f"Роль {role_key} удалена."
...
def detect_query(text: str, chat_id: int, admin_user: bool) -> str | None:
    lower = normalize_text(text)
    target_chat_id = None if admin_user else chat_id
    title_all = "Активные задачи:" if admin_user else "Мои активные задачи:"

    if "сегодня" in lower and "задач" in lower:
        return list_text(
            storage.list_reminders(only_today=True, target_chat_id=target_chat_id),
            "Задачи на сегодня:",
        )
    ...
    if "мои задачи" in lower or ("мои" in lower and "задачи" in lower):
        return list_text(
            storage.list_reminders(target_chat_id=chat_id),
            "Мои активные задачи:",
        )
    if admin_user and "все задачи" in lower:
        return list_text(storage.list_reminders(), title_all)
    return None
...
def detect_targets(text: str, author_chat_id: int) -> tuple[list[str], bool]:
    ...
    aliases_map = role_aliases()
    matches: list[tuple[int, str]] = []
    for key, member in team_map().items():
        aliases = set(aliases_map.get(key, []))
        aliases.add(key.lower())
        member_name = str(member.get("name", "")).lower()
        if member_name:
            aliases.add(member_name)
        positions = [lower.find(alias) for alias in aliases if alias and alias in lower]
        if positions:
...
def build_title(text: str, category: str) -> str:
    ...
    cleaned = re.sub(
        r"\b(manager|montaj|publisher|videograf|scriptwriter|designer|smm)\b",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
        r"\b(монтажер|монтажёр|мобилограф|видеограф|дизайнер|сценарист|менеджер)\b",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def create_natural_reminders(chat_id: int, text: str, allow_create: bool = True) -> str | None:
    lower = normalize_text(text)
    if lower.startswith("/"):
        return None

    query_reply = detect_query(text, chat_id, admin_user=allow_create)
    if query_reply:
        return query_reply
...
def parse_voice_text(voice_text: str, chat_id: int, allow_create: bool) -> str:
    result = create_natural_reminders(chat_id, voice_text, allow_create=allow_create)
    if result:
        return result
...
def handle_text_message(chat_id: int, text: str) -> None:
    ...
    if text == "/team":
        ...
        return
    if text == "/roles":
        bot.send_message(chat_id, roles_text())
        return
    if text == "/categories":
    ...
    if text == "/list":
        target_chat_id = None if admin_user else chat_id
        title = "Активные задачи:" if admin_user else "Мои активные задачи:"
        bot.send_message(chat_id, list_text(storage.list_reminders(target_chat_id=target_chat_id), title))
        return
    if text == "/today":
        target_chat_id = None if admin_user else chat_id
        bot.send_message(
            chat_id,
            list_text(storage.list_reminders(only_today=True, target_chat_id=target_chat_id), "Задачи на сегодня:"),
        )
        return
    if text == "/add":
        bot.send_message(
    ...
    if not admin_user:
        natural_result = create_natural_reminders(chat_id, text, allow_create=False)
        if natural_result:
            bot.send_message(chat_id, natural_result)
        else:
            bot.send_message(
                chat_id,
                "Ты можешь спросить так:\n"
                "- Какие задачи на сегодня?\n"
                "- Мои задачи\n\n"
                "Создавать задачи пока может только админ.",
            )
        return
    ...
    try:
        if text.startswith("/add "):
            ...
            bot.send_message(chat_id, add_role_from_command(text))
        elif text.startswith("/delrole "):
            bot.send_message(chat_id, remove_role_from_command(text))
            ...
            natural_result = create_natural_reminders(chat_id, text, allow_create=True)
            if natural_result:
                bot.send_message(chat_id, natural_result)
            else:
...
def handle_message(message: dict[str, Any]) -> None:
    ...
    if message.get("voice"):
        ...
        if (
            transcript.startswith("Ошибка")
            or transcript.startswith("Не удалось")
            or transcript.startswith("Голосовые")
        ):
            return
        bot.send_message(chat_id, parse_voice_text(transcript, chat_id, allow_create=is_admin(chat_id)))
        return
